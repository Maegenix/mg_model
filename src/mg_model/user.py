...
class User(Document):
    email = StringField(max_length=99, required=True)
    password = StringField(max_length=99, required=True)
    username = StringField(max_length=99)
    reference = StringField(default=f"User/{uuid.uuid4()}", required=True)
    player = StringField()
    online = BooleanField(default=False, required=True)
    resource = StringField(default="User", required=True)
    is_authenticated = BooleanField(default=True, required=True)
    last_login = DateTimeField(
        default=timezone.now(), verbose_name="last login", required=True
    )
    date_joined = DateTimeField(
        default=timezone.now(), verbose_name="date joined", required=True
    )
